...
int splicing_assignment_matrix_bias(const splicing_gff_t *gff, size_t gene,
  ...
  splicing_vector_int_destroy(&isop);
...
int splicing_paired_assignment_matrix_bias(const splicing_gff_t *gff,
					   size_t gene, int readLength, 
					   int overHang, int bias,
					   const splicing_vector_t 
					   *fragmentProb,
					   int fragmentStart, 
					   double normalMean,
  ...
  if (bias != 0) { 
    SPLICING_ERROR("Fragment bias is not supported for paired-end reads "
		   "and proper (=non-fast) assignment matrix", 
		   SPLICING_UNIMPLEMENTED);
  }
...
int splicing_paired_assignment_matrix(const splicing_gff_t *gff, size_t gene,
				      ...
				      double normalVar, double numDevs,
				      splicing_matrix_t *matrix) {
  return splicing_paired_assignment_matrix_bias(gff, gene, readLength, 
						overHang, /*bias=*/ 0, 
...
int splicing_paired_assignment_matrix_fast_bias(const splicing_gff_t *gff, 
				       size_t gene, int readLength, 
				       int overHang, int bias,
				       const splicing_vector_t *fragmentProb,
				       int fragmentStart, double normalMean,
  ...
  for (i=0; i<il; i++) {
    ...
    SPLICING_CHECK(splicing_assignment_matrix_bias(gff, gene, myrl, overHang, 
						   bias, &tmpmat));
...
int splicing_paired_assignment_matrix_fast(const splicing_gff_t *gff, 
				  size_t gene, int readLength, int overHang,
				  const splicing_vector_t *fragmentProb,
				  int fragmentStart, double normalMean,
				  double normalVar, double numDevs,
				  splicing_matrix_t *matrix) {
  return splicing_paired_assignment_matrix_fast_bias(gff, gene, readLength, 
						    overHang, /*bias=*/ 0,
						    fragmentProb, 
