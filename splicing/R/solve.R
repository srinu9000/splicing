...
runLinear <- function(geneStructure, readsfile,
                      ...
                      fragmentBias=c("none", "linear", "quadratic"),
                      verbose=TRUE, snowCluster=NULL, seed=NULL,
                      dropBadCigar=FALSE, ...) {

  fragmentBias <- match.arg(fragmentBias)
  rfunc <- switch(fragmentBias, "none"=solveIso, "linear"=solveIsoLinBias,
                  "quadratic"=solveIsoQuadBias)

  run(runFunction=rfunc, writeFunction=writeLinear,
...
solveIsoLinBias <- function(geneStructure, gene=1L, reads,
  ...
  sol0 <- nnls(t(1*am1 + am0), cbind(matvec))
  sol1 <- nnls(t(2*am1 + am0), cbind(matvec))
  ...
  optsol <- optimize(f=f, c(-1,1), tol=.Machine$double.eps,
